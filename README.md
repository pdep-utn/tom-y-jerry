...
Bien, ¡excelente! ya tenemos un objeto. Por otra parte dice que tenemos que manejar su energía y su velocidad dependiendo de actividades como comer ratones y de correr. Nos da una idea de lo que puede ser un atributo y lo que puede ser un método. Pero vamos despacio y sin apuro. Lo primero que tenemos que pensar es en qué mensajes debería entender nuestro objeto que representa a Tom. Leamos un poco más del requerimiento:
 
> Cuando tom come un ratón, su energía aumenta en (12 joules + el peso del ratón en gramos)
 
mmm, ok. Necesitamos que tom entienda el mensaje comer a un ratón. Por lo tanto podemos escribir el método en el objeto para luego describir el comportamiento que va a tener. Entonces:
```wollok
object tom {
...
1. **Acción** son los métodos que producen un cambio en el estado del objeto. Es decir, si yo tengo mi objeto con una referencia a otro objeto (por ejemplo un número: el 3), si alguien envía un mensaje cuyo método tiene el comportamiento de cambiar dicha referencia por otro valor (podemos suponer para ejemplo otro número: el 5) entonces se produce ese cambio de referencia **sin retornar valor alguno**.
...
En este caso, ¿de qué tipo de método se trata? Es de acción, porque debemos incrementar su energía en un determinado valor. Ok, entonces sabemos que se incrementa en 12 joules + el peso del ratón. Veamos cómo traducimos esto a código
```wollok
object tom {
...
Por el momento y a fines didácticos nos vamos a tomar la libertad de inicializarla con el valor 0, de lo contrario el IDE nos va a marcar que no tenemos inicializada la variable energía. Esta es una decisión que no podemos tomar de forma deliberada como desarrolladores: podemos inicializar un objeto en un estado que no es el esperado. ¿Cuál es la solución? comunicarse con el cliente y consultar.
 
Vamos a hacer foco en cómo obtenemos el peso del ratón. Veamos diferentes opciones
...
```
Ok, esto funciona. Pero, ¿es una buena solución? Mmmm... en primer lugar tengo ratón sin referenciar, entonces necesitamos referenciar a jerry por ejemplo. Lo hacemos a través de un método:
```wollok
object tom {
 var energia = 0
 var raton
...
```
Este tipo de método de **acción** se conoce como **setter** porque "setea" una referencia del objeto. También podríamos tener un método de **pregunta** que nos devuelva el valor de la variable y estos se conocen como **getters**.
```wollok
object tom {
 var energia = 0
 var raton
...
### ¿Cuanto pesás?
Otra opción es que pasemos el peso como parámetro, es decir que tengamos una referencia con un scope reducido al método que estamos modelando.
```wollok
object tom {
 var energia = 0
...
Entonces nuevamente tenemos que pensar qué mensaje nos tiene que llegar a tom. Correr puede ser un buen nombre para el mensaje y recibe un tiempo que corre en segundos. Entonces :
...
¡Bien! ya tenemos la interfaz, es decir la firma del método (que recibe, cómo se llama y qué devuelve). Nuevamente estamos ante un método de **acción** porque tenemos que cambiar el estado del objeto, es decir que tenemos que disminuir su energía.
...
```
Ok, generamos un método cohesivo que se encarga de asignar el valor correspondiente a la velocidad. Pero pueden notar que debo siempre invocar al método **self.calcularVelocidad()** para que quede referenciada la velocidad que necesito. Si la velocidad estuviese utilizada desde otros métodos voy a tener que **siempre** llamar antes a este método repitiendo esta lógica a lo largo de todo el objeto.
...
object tom {
 ...
 method correr(segundos) {
   self.calcularVelocidad()
   const espacio = self.velocidad() * segundos
   energia = energia - coeficienteDeEnergiaAlCorrer * espacio
 }
}
```
...
¿Es de pregunta o acción? Claramente es de pregunta porque me tiene que dar true si le conviene. Entonces no tiene que producirse un cambio en el estado de tom. ¿Qué comportamiento le tenemos que poner en el método?
 
¿self.comer(unRaton) > self.correr(unaDistancia)?
 
Tanto comer como correr son mensajes de acción y no de pregunta, es decir que no me devuelven un valor sino que modifican el estado del objeto, cosa que yo no quiero!!!!
 
Esto en funcional no era un problema porque siempre era tom. Cuando corría tenía otro tom con menos energía pero mi tom original era siempre el mismo. Ahora este beneficio no lo tenemos. Entonces nuestros métodos que son de acción podemos dividirlo en dos partes: La acción propiamente dicha y otra abstracción para saber la energía que modifica. Entonces
...
object tom {
 ...
 meConvieneComerRatonA(unRaton, unaDistancia) = self.energiaPorComer(unRaton) > self.energiaPorCorrer(unaDistancia)
}
```
 
Entonces necesitamos esas abstracciones
...
object tom {
 ...
 method correr(segundos) {
   self.calcularVelocidad()
   const espacio = self.velocidad() * segundos
   energia = energia - self.energiaPorCorrer(espacio)
 }
 
 method energiaPorCorrer(unaDistancia) = coeficienteDeEnergiaAlCorrer * unaDistancia
 
 meConvieneComerRatonA(unRaton, unaDistancia) = self.energiaPorComer(unRaton) > self.energiaPorCorrer(unaDistancia)
}
```
 
Y lo mismo con la energía por comer
...
object tom {
 ...
 method energiaPorComer(unRaton) = energiaBase + unRaton.peso()
